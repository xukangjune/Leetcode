"""
对所有的区间先进行排序，排序的值为第一个数，接下来是第二个数。这样的话从第一位开始遍历列表。如果后一个区间的头大于等于前一个区间的尾，说明是独立的
区间。反之，就是重叠的部分，那么下面要做的就是删除其中一个，其实就是比较这两个范围，谁更”广“一点。首先。比较这两个区间的尾，如果前一个区间的尾小于后
一个区间，那么前面区间的要”广“一些，删除前面一个，具体的做法为将后一个设为下一次遍历的前一个；反之，说明后一个区间”广“一点，那么就得删除后一个，具体
操作为下次遍历的前一个区间不变。
"""
...
print(solve.eraseOverlapIntervals(nums))